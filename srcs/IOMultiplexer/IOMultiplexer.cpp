...
void Select::set_timeout(int timeout_msec) {
    if (timeout_msec <= 0) {
        DEBUG_PRINT(CYAN, "select set_timeout: [%d]->[-]sec", this->timeout_.tv_sec + this->timeout_.tv_usec/1000000);
        this->timeout_.tv_sec = 0;
        this->timeout_.tv_usec = 0;
    } else {
        DEBUG_PRINT(CYAN, "select set_timeout: [%d]->[%d]sec", this->timeout_.tv_sec + this->timeout_.tv_usec/1000000, timeout_msec/1000);
        this->timeout_.tv_sec = timeout_msec / 1000;
