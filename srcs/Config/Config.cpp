								...
								std::vector<std::vector<std::string> > *server_name_list)
{
	if (IsConfigFormat::is_start_location_block(config_line) == CONFIG_FORMAT_OK)
	{
		*in_location_block = true;
		return OK_START_LOCATION_BLOCK;
	}
	...
	if (action_result == CONFIG_FORMAT_OK)
	{
		...
		if (done_input_action == CONFIG_FORMAT_OK)
			return OK_IN_SERVER_BLOCK;
		return (ALREADY_EXIST_FIELD_KEY);
	}
								 ...
								 std::vector<std::vector<std::string> > *server_name_list)
{
	...
	while (std::getline(config_lines, config_line, '\n'))
	{
		...
		if (!in_server_block && !in_location_block)
		{
			...
			return this->report_errorline(config_file_name, config_line, line, result);
		}
		if (in_server_block && !in_location_block)
		{
			...
			return this->report_errorline(config_file_name, config_line, line, result_server_block_action);
		}
		if (in_server_block && in_location_block)
		{
			if (IsConfigFormat::is_block_end(config_line))
		...
		return this->report_errorline(config_file_name, config_line, line, NOT_ALLOWED_CONFIG_FORMAT);
	}
	if (in_server_block || in_location_block) {
		return this->report_errorline(config_file_name, config_line, line, NOT_END_CONFIG);
	}
	return true;
}
...
void Config::init_location_config_with_server_config(LocationConfig *location_config,
													 ...
													 bool *in_server_block)
{
	location_config->init_location_config();

	ServerConfig server_config = this->get_allconfig(server_names).get_server_config();
	location_config->init_location_config_with_server_config(server_config);
	...
	while (std::getline(config_lines, config_line, '\n'))
	{
		...
		if (in_server_block && in_location_block)
		{
			if (IsConfigFormat::is_block_end(config_line))
			{
				...
				location_config.init_location_config();
				location_config.init_location_config_with_server_config(
						this->get_allconfig(*server_name_itr).get_server_config());
				in_location_block = false;
				continue;
			}
			...
			return this->report_errorline(config_file_name, config_line, line, NOT_LOCATION_CONFIG_FORMAT);
		}
		return this->report_errorline(config_file_name, config_line, line, NOT_ALLOWED_CONFIG_FORMAT);
	}
	return true;
...
bool Config::get_is_config_format() const { return this->_is_config_format; }
...
AllConfig Config::get_allconfig(const std::vector<std::string> &server_names)  // これがconfigの読み取りの時に使うやつ
{
	return (this->_all_configs[server_names]);
}

AllConfig Config::get_allconfig(const std::string &server_name)  // これがrequestように使うやつ
{
...
std::string get_error_msg(int error_type) {
	std::string err_msg;

	switch (error_type)
	{
		case NO_FIELD_HEADER:
			err_msg = "NO_FIELD_HEADER";
			break;
		case NO_FIELD_VALUE:
			err_msg = "NO_FIELD_VALUE";
			break;
		case NO_LAST_SEMICOLON:
			err_msg = "NO_LAST_SEMICOLON";
			break;
		case NO_SEMICOLON:
			err_msg = "NO_SEMICOLON";
			break;
		case MULTIPLE_SEMICOLON:
			err_msg = "MULTIPLE_SEMICOLON";
			break;
		case NOT_ENDWORD_EXIST:
			err_msg = "NOT_ENDWORD_EXIST";
			break;
		case NOT_START_BLOCK:
			err_msg = "NOT_START_BLOCK";
			break;
		case NOT_EXIST_KEYWORD_SERVER:
			err_msg = "NOT_EXIST_KEYWORD_SERVER";
			break;
		case NOT_EXIST_KEYWORD_LOCATION:
			err_msg = "NOT_EXIST_KEYWORD_LOCATION";
			break;
		case NOT_ENDWORD_CURLY_BRACES:
			err_msg = "NOT_ENDWORD_CURLY_BRACES";
			break;
		case SERVER_BLOCK_KEY_ALREADY_EXIST:
			err_msg = "SERVER_BLOCK_KEY_ALREADY_EXIST";
			break;
		case LOCATION_BLOCK_KEY_ALREADY_EXIST:
			err_msg = "LOCATION_BLOCK_KEY_ALREADY_EXIST";
			break;
		case NOT_FIELD_KEY_PRINTABLE:
			err_msg = "NOT_FIELD_KEY_PRINTABLE";
			break;
		case NOT_CURLY_BRACES_EXIST:
			err_msg = "NOT_CURLY_BRACES_EXIST";
			break;
		case NOT_ONLY_CURLY_BRACES:
			err_msg = "NOT_ONLY_CURLY_BRACES";
			break;
		case NOT_LAST_WARD_SEMICOLON:
			err_msg = "NOT_LAST_WARD_SEMICOLON";
			break;
		case NOT_SERVER_CONFIG_FORMAT:
			err_msg = "NOT_SERVER_CONFIG_FORMAT";
			break;
		case NOT_LOCATION_CONFIG_FORMAT:
			err_msg = "NOT_LOCATION_CONFIG_FORMAT";
			break;
		case ALREADY_EXIST_FIELD_KEY:
			err_msg = "ALREADY_EXIST_FIELD_KEY";
			break;
		case NOT_ALLOWED_SERVER_BLOCK_FORMAT:
			err_msg = "NOT_ALLOWED_SERVER_BLOCK_FORMAT";
			break;
		case NOT_ALLOWED_CONFIG_FORMAT:
			err_msg = "NOT_ALLOWED_CONFIG_FORMAT";
			break;
		case NOT_END_CONFIG:
			err_msg = "NOT_END_CONFIG";
			break;
		case TOO_MANY_CURLY_BRACES:
			err_msg = "TOO_MANY_CURLY_BRACES";
			break;
		case NOT_PRINTABLE:
			err_msg = "NOT_PRINTABLE";
			break;
		case NOT_FIELD_VALUE_FORMAT:
			err_msg = "NOT_FIELD_VALUE_FORMAT";
			break;
		case NOT_START_CURLY_BRACES:
			err_msg = "NOT_START_CURLY_BRACES";
			break;
		case FORBIDDEN_WORD:
			err_msg = "FORBIDDEN_WORD";
			break;
		default:
			break;
	}
