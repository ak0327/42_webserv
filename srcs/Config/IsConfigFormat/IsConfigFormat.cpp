...
bool	IsConfigFormat::is_start_locationblock(const std::string &line, std::string &config_location_path)
{
	...
	if (!HandlingString::is_printable_content(location_path) || end_pos == line_without_ows.length())  // location pathに関して本当にprintaleかどうかのチェックだけでいいのか
		return (false);
	while (HandlingString::is_ows(line_without_ows[end_pos]))
		end_pos++;
	start_pos = end_pos;
...
{
	std::string		line_without_ows = HandlingString::obtain_without_ows_value(line); // (文字列->header) <OWS> {文字列->value};
	size_t			start_pos = 0;
	size_t			end_pos = 0;
	...
	// 	return (false);
	if (line_without_ows.length() == end_pos || line_without_ows.length() != ++end_pos)
		return (false);
	field_value = line_without_ows.substr(start_pos, end_pos - 1);
...
bool	IsConfigFormat::is_serverblock_format(const std::string &line, bool &in_server_block, bool &in_location_block, ServerConfig *serverconfig, std::string &location_path)
{
	std::string		line_without_ows = HandlingString::obtain_without_ows_value(line); // (文字列->header) <OWS> {文字列->value};
	size_t			start_pos = 0;
	size_t			end_pos = 0;
